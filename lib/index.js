...
async function singleSearch(options) {
  ...
  try {
    const filter = new ldap.filters.EqualityFilter({
      attribute: options.userSearchAttribute,
...
/**
 * Search for an user in a LDAP server. Resolves to the user if bind is
 * successful (information may be incomplete) or null otherwise.
 * @param {import('../types').BindOptions} options
 * @returns {Promise<import('ldapjs').SearchEntryObject | null>}
 */
async function bindSearch(options) {
  try {
    const client = await _bindLdap(options, false);
    try {
      const user = await _searchUser(client, options.dn);
      return user || { dn: options.dn, controls: [] };
    } finally {
      client.unbind();
    }
  } catch (err) {
    if (err instanceof ldap.InvalidCredentialsError) {
      throw new InvalidUserCredentialsError(options.dn, false);
    } else {
      throw err;
    }
  }
 ...
 * @returns {Promise<import('ldapjs').SearchEntryObject | null>}
 */
function _searchUser(ldapClient, baseDn, filter) {
  return new Promise((_resolveInternal, _rejectInternal) => {
    let done = false;
    function reject(reason) {
      if (!done) {
    ...
    function resolve(val) {
      ...
      debug('Search successful,', val ? 'user found.' : 'but no user found.');
      _resolveInternal(val);
    }
